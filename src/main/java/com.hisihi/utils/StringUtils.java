...
/**
 * Created by andyYang on 2015/6/14.
 */
public class StringUtils {
    ...
    private static SimpleDateFormat format = new SimpleDateFormat(
            "yyyy-MM-dd HH:mm:ss");
    private static SimpleDateFormat format_cn = new SimpleDateFormat(
            "yyyy��MM��dd��  HHʱmm��ss��");
    ...
    public static String getRequestParams(HttpServletRequest request) {
        ...
        while (enu.hasMoreElements()) {
            ...
            paramBuffer.append(paraName).append("=")
                    .append(request.getParameter(paraName));
            paramBuffer.append("&");
        }
    ...
    public static String getPushListRedisKey(String appId, String type,
                                             String send_source) {
    ...
    public static String getEmailToken(String emial) {
        String token = getRandomChars(3) + "-" + getRandomChars(3) + "-"
                + toMD5(StringUtils.getUUID() + emial).substring(2, 22);
        return token;
    }

    public static String getJsCNParameter(String para){
        try {
            para = URLDecoder.decode(URLEncoder.encode(para, "ISO-8859-1"), "UTF-8");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        return para;
